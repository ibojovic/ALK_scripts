...
STARfusion <- read.table("~/data/starfusion_output/STARfusion_out_known_ALK_data.txt", header = TRUE, sep = "\t")
arriba <- read.table("~/data/arriba_output/arriba_known_ALK_data.txt", header = TRUE, sep = "\t")
fcirc <- read.table("~/data/fcirc_output/fcirc_known_ALK_data.txt", header = TRUE, sep = "\t")
infusion <- read.table("~/data/infusion_output/infusion_known_ALK_data.txt", header = TRUE, sep = "\t")
fusioncatcher <- read.table("~/data/fusioncatcher_output/fusioncatcher_known_ALK_data.txt", header = TRUE, sep = "\t")
jaffa_d <- read.table("~/data/jaffa_output/jaffa_d_known_ALK_data.txt", header = TRUE, sep = "\t")
squid<-read.table("~/data/squid_output/squid_known_ALK_data.txt", header = TRUE, sep = "\t")
...
emerald_palette <- c("#006633", "#339966", "#66cc99", "#99ffcc")

# Plot
p <- ggplot(complete_counts, aes(x = table_name, y = log_count, fill = cell_line)) +
  geom_bar(stat = "identity", position=position_dodge(width=0.9)) +
  scale_fill_manual(values = c(emerald_palette)) +
  theme_minimal() +
...
ggplot(complete_counts, aes(x = table_name, y = cell_line)) +
  ...
  scale_fill_gradient(low = "white", high = "#339966") + # You can change the color scheme
  theme_minimal() +
